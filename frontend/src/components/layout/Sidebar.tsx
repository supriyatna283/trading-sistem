...
export default function Sidebar({ onWidthChange, mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-50 flex flex-col h-screen border-r border-white/5 bg-[#080c14]/95 backdrop-blur-xl transition-all duration-300 ease-in-out shadow-[10px_0_30px_rgba(0,0,0,0.5)] overflow-hidden ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
          }`}
        style={{ width: w }}
      >
        {/* ── Logo ── */}
